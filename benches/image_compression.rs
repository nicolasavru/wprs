...
fn compress_png(c: &mut Criterion, path: &Path) -> f64 {
    ...
    println!("compression rate (lower is better): {:.1}%", compression_rate * 100.0);
    ret
}
...
fn compression_benchmark(c: &mut Criterion) {
    ...
    println!("mean compression with filter ratio (higher is better): {mean_filter_compression_ratio:.1}");
    println!(
